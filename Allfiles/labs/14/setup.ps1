...
if (-not $Region)
{
    ...
    $locations = Get-AzLocation | Where-Object {
        $_.Providers -contains "Microsoft.Synapse" -and $_.Providers -contains "Microsoft.Sql" -and $_.Providers -contains "Microsoft.Storage" -and $_.Providers -contains "Microsoft.Compute" -and $_.Providers -contains "Microsoft.DocumentDB" -and $_.Location -in $preferred_list
    }
...
$synapse = Get-AzSynapseWorkspace -Name $config.synapseWorkspace -ResourceGroupName $resourceGroupName -ErrorAction SilentlyContinue
if (-not $synapse)
{
...
Write-Host "Creating Cosmos DB account..."; # Try the same region as Synapse, and if that fails try others...
$cosmosDB = $config.cosmosDB
if ($cosmosDB)
{
    $cosmosDB = Get-AzCosmosDBAccount -ResourceGroupName $resourceGroupName -Name $config.cosmosDB -ErrorAction SilentlyContinue
}
if (-not $cosmosDB)
{
    $stop = 0
...
{
    Write-Host "Cosmos DB $cosmosDB already exists."
}
